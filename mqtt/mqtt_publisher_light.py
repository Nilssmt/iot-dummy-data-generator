...
from time_interval import frequent_send_light
...
def check_event(data):
    ...
    time_value = frequent_send_light
...
light_data_list = load_csv_data('./CSV_puffer/light_iot_sensor_data.csv')
...
try:
    for i in range(len(light_data_list)):
        event = check_event(light_data_list[i])
        time.sleep(event)
        if event == 0:
            client.publish("iot/sensor/data/light", payload=pair_data_with_timestamp(light_data_list[i]), qos=1)
        print(f"Sent data: {light_data_list[i]} \n")
